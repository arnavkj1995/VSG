...
class WorldModel(tools.Module):
  ...
  def __init__(self, step, config):
    ...
    elif config.model_type == 'srssm':
       self.dynamics = networks.StochasticRSSM(
            config.dyn_stoch, config.dyn_hidden, 
            config.dyn_input_layers, config.dyn_output_layers,
            config.dyn_rec_depth, config.dyn_shared, config.dyn_discrete,
            config.act, config.dyn_mean_act, config.dyn_std_act,
            config.dyn_temp_post, config.dyn_min_std, config.dyn_cell)
  ...
  def train(self, data):
    ...
    with tf.GradientTape() as model_tape:
      ...
      kl_loss, kl_value = self.dynamics.kl_loss(
          post, prior, self._config.kl_forward, kl_balance, kl_free, kl_scale, self._config.dyn_kl_mask)

      if self._config.dyn_gate_scale > 0.0:
          sparsity_loss = self.dynamics.sparsity_loss(
              post,
              self._config.dyn_gate_prior,
              self._config.dyn_gate_free,
              self._config.dyn_gate_scale,
          )
      else:
          sparsity_loss = 0.0
      ...
      model_loss = sum(losses.values()) + kl_loss + sparsity_loss
    ...
    if self.dynamics._is_cell_stochastic:
        metrics["sparsity_loss"] = sparsity_loss
        metrics["sparsity_c_mean"] = tf.reduce_mean(
            tf.reduce_sum(post["u_sample"], axis=-1)
        )
        metrics["sparsity_t_mean"] = tf.reduce_mean(
            tf.reduce_sum(post["u_sample"], axis=-2)
        )
        metrics["sparsity_c_std"] = tf.math.reduce_std(
            tf.reduce_sum(post["u_sample"], axis=-1)
        )
        metrics["sparsity_t_std"] = tf.math.reduce_std(
            tf.reduce_sum(post["u_sample"], axis=-2)
        )
...
class ImagBehavior(tools.Module):
  ...
  def train(
    ...
    with (tape or tf.GradientTape()) as actor_tape:
      ...
      state_ent = tf.reduce_sum(
              self._world_model.dynamics.get_dist(imag_state, tf.float32).entropy(),
              axis=-1,
          )
